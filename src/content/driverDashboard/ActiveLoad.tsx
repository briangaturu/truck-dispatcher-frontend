  ...
  if (isLoading) {
    return (
      <div className="dashboard-card">
        <div className="p-10 text-center text-slate-500 text-sm">Loading active load...</div>
      </div>
    );
  }
  ...
  return (
    <div className="dashboard-card">
      <div className="dashboard-card-header">
        <div>
          <h3 className="dashboard-card-title">Active Load</h3>
          <p className="dashboard-card-subtitle">Your current assignment</p>
        </div>
      </div>
      <div className="grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-4">
        ...
        <div className="flex flex-col gap-1"><strong className="text-[10px] text-slate-400 uppercase tracking-wider">Status</strong>
          <span className="status-badge" style={{ background: "#eff6ff", color: "#2563eb" }}>In Transit</span>
        </div>
      </div>
    </div>
  );
